...
Texture2D    matTex							: register( t2 ) ;		// マットキャップ
...
// ライト5の処理 +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++( 終了 )
    ...
    float3 cameraDir = PSInput.VPosition.xyz - cameraPos.xyz;
	//正規化
    cameraDir.xyz = Normalize(cameraDir.xyz);
	//法線方向との内積からアウトラインを作成するかを決める
    float3 cross = Cross(cameraDir.xyz, PSOutput.Normal.xyz);
    if (Length(cross) <= 0.9)
    {
        PSOutput.Color0.rgb *= 2;
    }
